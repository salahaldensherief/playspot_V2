...
- **🆕 Router Singleton — Never Re-instantiated:** The `GoRouter` instance (and the `GoRouter(...)` configuration that builds it) MUST be constructed exactly once for the app's lifetime — as a `static final` / top-level field, or registered in GetIt — and consumed via that single reference everywhere (`MaterialApp.router(routerConfig: ...)`). It must NEVER be created inside a widget's `build()` method (e.g. `AppRouter().router` called directly in `MyApp.build()`), because any rebuild of that widget (locale change, `ScreenUtilInit` resize, theme change, etc.) would silently reset the entire navigation stack.
- **🆕 Route Existence Guarantee:** Before writing or approving any `context.go(...)`, `context.goNamed(...)`, or `context.push(...)` call that references a `RouterKeys.X` constant, verify that a matching `GoRoute` with that exact `name`/`path` is actually registered in the router configuration. A navigation call to an unregistered route is a build-blocking defect, not a runtime edge case — it must be caught before merge, not discovered by a user tapping a button.
...
- **🆕 Forms Must Actually Bind Input:** Any screen containing input fields (`AppTextField` or otherwise) MUST wire every field to either a `TextEditingController` owned by the relevant Cubit, or an explicit `onChanged` callback that updates Cubit/local state. A form that renders fields but collects no data (no controller, no `onChanged`) is a build-blocking defect — treat it with the same severity as a crash, since it fails silently in QA and only surfaces as "the button does nothing" in production.
...
- **🆕 No Derived Data Recomputed Per Frame:** Any list, map, or `.tr()`-translated collection that does not depend on the current build's parameters (e.g. static onboarding slide data) MUST be computed once — as a `final` field, in `initState`, or hoisted to a top-level constant — never re-created inline on every `build()` call, including builds triggered by unrelated `setState` calls (e.g. a `PageView` page-change indicator).
- **🆕 No Artificial Fixed Delays Gating Navigation:** Splash/bootstrap screens must not gate navigation behind a hardcoded `Future.delayed` duration used as a substitute for real initialization. The delay (if any, for branding purposes) must be the *minimum* of a short cosmetic duration and the actual completion of real bootstrap work (auth/session check, config/cache warm-up) via `Future.wait`, not a blind fixed wait unrelated to real readiness.
...
- **🆕 No Partial Scaffolding — Migrate, Don't Leave Half-Applied:** A feature that currently only has `data/` and `presentation/` (missing `domain/`, with the Cubit calling the Repository directly and no Usecase layer) is **out of compliance**, not a legacy exception. The next task that touches that feature MUST introduce the missing `domain/` layer (abstract repository interface + entities + usecases) as part of the change, rather than adding more code on top of the incomplete structure. Do not scaffold new features on the old (data+presentation-only) pattern going forward — every new feature starts with the full 3-layer structure from day one.
- **🆕 Folder Name Consistency Note:** The current directory is spelled `presetation/` in the existing codebase (missing an "n"). Do not propagate this typo into new features — new feature folders must use the correct `presentation/` spelling. Renaming the existing folder is a separate, deliberate refactor (it touches every import in that feature) and should be scheduled explicitly, not done incidentally inside an unrelated task.
...
- **🆕 No Unused Constructor Parameters or Fields:** If a class declares a constructor parameter or a field, it MUST be read/used somewhere in that class's logic. A parameter that is accepted but never referenced (e.g. a widget taking a `Cubit` it never calls) is treated as a defect to fix immediately, not a stylistic nitpick — it usually signals a form or callback that silently isn't wired up.
...
- **CUSTOM COMPONENT REUSE FOR REPEATED UI:** Any UI widget, card, list tile, input field, status badge, dialog, or bottom sheet that appears in more than one place MUST be created as a custom reusable widget component under `lib/art_core/widgets/` or feature-specific `widgets/`. Re-writing or duplicating identical UI structures across screens is strictly forbidden. **Two occurrences is the trigger, not three** — the moment the same structure (a spacer, a gradient button config, a card layout) is written a second time, it must be extracted before a third copy is allowed to exist.
...
- **🆕 No Near-Duplicate Design Tokens:** Before adding a new color, gradient, or shadow constant, search the existing theme file for an equivalent value. Do not define a second "primary gradient" or "brand blue" with slightly different hex values than one that already exists (e.g. two different blue→purple gradients used interchangeably across screens) — this produces visible inconsistency across the app. If an existing token is close but not exact, either reuse it or explicitly rename/version it (e.g. `primaryGradientV2`) with a comment explaining why both must coexist.
...
- **🆕 `Either<String, T>` Is Non-Compliant:** Returning `Either<String, T>` from a repository (rather than `Either<Failure, T>`) and building the string via `Left(e.toString())` is explicitly prohibited — it discards the ability to distinguish network, auth, and validation failures in the presentation layer and cannot be localized. Any existing repository method written this way must be migrated to the `Failure` hierarchy the next time it is touched.
...
- **🆕 No Commented-Out Code:** Do not leave dead code blocks commented out "just in case" (old feature logic, leftover fields from a previous project template, disabled function bodies). Delete it — git history is the correct place to recover it if ever needed. A file with large commented-out sections is treated the same as a file with leftover `print()` debugging: it must be cleaned before the task is considered done.
...
- **🆕 No Placeholder Secrets Beyond a Feature Branch:** A literal placeholder value (e.g. `'YOUR_ANON_KEY'`) is acceptable only as a momentary local stub while actively wiring a new integration. It must never reach `main`/`develop` or any shared branch — treat an unreplaced placeholder credential as a release blocker, since it means the corresponding feature (in this case, all of Supabase auth) is silently non-functional.
...
- **🆕 Consistent Constant Casing Within a Class:** A single class holding a set of constants (e.g. a caching-key registry) MUST use one consistent casing convention for all its members — either all `camelCase` or all `SCREAMING_SNAKE_CASE`. Mixing both in the same class (e.g. `TOKEN` alongside `FullName`) is prohibited; pick `camelCase` project-wide for consistency with standard Dart style, and migrate mixed-case registries the next time they're touched.
- **🆕 Extensions Must Target the Narrowest Real Type:** Extension methods/getters must be declared on the specific type they operate on (`String?`, `List<T>?`, `Map<K, V>?`, a specific model class, etc.). Extensions declared `on dynamic` or `on Object` are prohibited — they silently apply to every value in the codebase, defeat static type checking, and make it impossible to know from an import list what surface area an extension actually affects.
...
- **Explicit `pubspec.yaml` Declaration:** Any package imported and used directly in Dart source code MUST be explicitly listed under `dependencies` (or `dev_dependencies` for test-only packages) in `pubspec.yaml`. Relying on a package that only appears in `pubspec.lock` — pulled in transitively as another package's own dependency — is prohibited, even if it currently resolves and compiles. A parent package is free to drop or version-bump that transitive dependency at any time, silently breaking the build with no local warning.
- **Version Pin Sanity:** When adding a new direct dependency, use the same caret-range convention (`^x.y.z`) already used throughout `pubspec.yaml`, and check for an existing package already in the project that covers the same need before adding a new one (e.g. two different HTTP/date-formatting/state-management libraries should not coexist without a documented reason).
